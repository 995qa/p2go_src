...
//			MessageEnd();
			...
			CUsrMsg_PaintEntity msg;
			msg.set_paintableent(pPaintedEntity->entindex());
			msg.set_paintpower(newPowerType);
			msg.set_posx(vecPosition.x);
			msg.set_posy(vecPosition.y);
			msg.set_posz(vecPosition.z);
			SendUserMessage(filter, UM_PaintEntity, msg);
		}
	}
	else
	{
...
//	MessageEnd();
	...
	CUsrMsg_RemoveAllPaint msg;
	msg.set_dummy(1);
	SendUserMessage(filter, UM_RemoveAllPaint, msg);
}
...
//		UserMessageBegin( filter, "PaintWorld" );
		...
		msg.set_brushentity(pBrushEntity->entindex());


		//WRITE_FLOAT( currentLocation->flPaintRadius );
		//WRITE_FLOAT( currentLocation->flPaintAlphaPercent );
		msg.set_paintradius(currentLocation->flPaintRadius);
		msg.set_alphapercent(currentLocation->flPaintAlphaPercent);
		...
		msg.set_paintcount(paintLocationCount);
		...
		msg.set_centerx(center.x);
		msg.set_centery(center.y);
		msg.set_centerz(center.z);
		...
		for( int j = 0; j < paintLocationCount; ++currentLocation, ++j )
		{
			...
			msg.set_contactpointx(offset.x);
			msg.set_contactpointy(offset.y);
			msg.set_contactpointz(offset.z);
		}

		// End the message
		//MessageEnd();
		SendUserMessage(filter, UM_PaintWorld, msg);
	}
...
void CC_PaintAllSurfaces( const CCommand& args )
{
	...
	// SanyaSho: protobuf
	CUsrMsg_PaintAllSurfaces msg;
	msg.set_paintpower( power );
	SendUserMessage( filter, UM_PaintAllSurfaces, msg );
}
