...
public:
	...
	bool MsgFunc_CreditsPortalMsg( const CUsrMsg_CreditsPortalMsg &msg );
	bool MsgFunc_LogoTimeMsg( const CUsrMsg_LogoTimeMsg &msg );
...
private:
	CUserMessageBinder m_UMCMsgCreditsPortalMsg;
	...
	void DrawTexture(const char *texturePath);
	void DrawLogo( void );
...
void CHudPortalCredits::DrawPortalOutroCreditsLyrics( void )
{
	...
	if (m_bStartSong && flCurTime>= m_flSongStartTime)
	{
		...
		DrawTexture("vgui/screens/vgui_single_credits_screen_bg.vtf");
		m_bStartSong=false;
...
void CHudPortalCredits::DrawTexture(const char *texturePath)
{
	Msg("TODO!\n");
	Msg("Overlaying texture: %s\n", texturePath);
...
bool CHudPortalCredits::MsgFunc_CreditsPortalMsg( const CUsrMsg_CreditsPortalMsg &msg )
{
...
bool CHudPortalCredits::MsgFunc_LogoTimeMsg( const CUsrMsg_LogoTimeMsg &msg )
{
